...
public class JsonStateWriter extends StateWriter {
	...
	private Map<String, Value> fcnApplies = new HashMap<>();

	public Set<String> varLookups = new HashSet<>();
	public Set<String> unchangedVars = new HashSet<>();
	...
	public void trackFcnApply(String arg, Value val) {
		this.fcnApplies.put(arg, val);
	}

	public void flushFcnApply() {
		this.fcnApplies.clear();
	}
	...
	public synchronized void writeState(final TLCState state) {
		...
		flushFcnApply();
		this.varLookups.clear();
		this.unchangedVars.clear();
	}
	...
	private synchronized void writeState(TLCState state, TLCState successor, BitVector actionChecks, int from, int length, short stateFlags,
		...
		Set<String> readVars = new HashSet<>(varLookups);
		readVars.removeAll(unchangedVars);
		...
		action.getOpDef().walkGraph(new Hashtable<>(), new ExplorerVisitor() {
			@Override
			public void postVisit(ExploreNode exploreNode) {
				if (exploreNode.getClass() == OpApplNode.class) {
					semanticNodes.add((SemanticNode) exploreNode);
				}
		...
		action.getOpDef().walkGraph(new Hashtable<>(), new ExplorerVisitor() {
			...
			public void postVisit(ExploreNode exploreNode) {
				 ...
				 */
				if (exploreNode.getClass() == OpApplNode.class) {
					OpApplNode node = (OpApplNode) exploreNode;
					boolean isPrimed = node.getOperator().getName().toString().equals("'");
					if (isPrimed) {
						for (SemanticNode sm : node.getChildren()) {
							if (sm.getClass() == OpApplNode.class) {
								OpApplNode d = (OpApplNode) sm;
								String name = d.getOperator().getName().toString();
								writes.add(name);
							}
						}
					}
				}
				if (exploreNode.getClass() == OpDeclNode.class) {
					OpDeclNode d = (OpDeclNode) exploreNode;
			...
			public boolean shouldVisitDescendants(ExploreNode exploreNode) {
				// we dont want to visit descendents of UNCHANGED nodes because otherwise we would treat them
				// as reads
				if (exploreNode.getClass() == OpApplNode.class) {
					OpApplNode node = (OpApplNode) exploreNode;
					boolean isUNCHANGEDNode = node.getOperator().getName().toString().equals("UNCHANGED");
					return !isUNCHANGEDNode;
				}
				return true;
			}
		});
